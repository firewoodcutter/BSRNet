...
def get_im_gt_name_dict(datasets, flag='valid'):
    ...
    for i in range(len(datasets)):
        ...
        tmp_im_list, tmp_gt_list, tmp_mask_list= [], [], []
        ...
        #mask
        if(datasets[i]["mask_dir"]==""):
            print('-mask-', datasets[i]["name"], datasets[i]["mask_dir"], ': ', 'No Mask Found')
            tmp_mask_list = []
        else:
            tmp_mask_list = [datasets[i]["mask_dir"]+os.sep+x.split(os.sep)[-1].split(datasets[i]["im_ext"])[0]+datasets[i]["mask_ext"] for x in tmp_im_list]

            # lbl_name_dict[im_dirs[i][0]] = tmp_gt_list
            print('-mask-', datasets[i]["name"],datasets[i]["mask_dir"], ': ',len(tmp_mask_list))   
        ...
        if flag=="train": ## combine multiple training sets into one dataset
            if len(name_im_gt_list)==0:
                name_im_gt_list.append({"dataset_name":datasets[i]["name"],
                                        "im_path":tmp_im_list,
                                        "gt_path":tmp_gt_list,
                                        "mask_path":tmp_mask_list,
                                        "im_ext":datasets[i]["im_ext"],
                                        "gt_ext":datasets[i]["gt_ext"],
                                        "mask_ext":datasets[i]["mask_ext"],
                ...
                name_im_gt_list[0]["mask_path"] = name_im_gt_list[0]["mask_path"] + tmp_mask_list
                if datasets[i]["im_ext"]!=".jpg" or datasets[i]["gt_ext"]!=".png" or datasets[i]["mask_ext"]!=".png":
                ...
                name_im_gt_list[0]["mask_ext"] = ".png"
...
def create_dataloaders(name_im_gt_list, cache_size=[], cache_boost=True, my_transforms=[], batch_size=1, shuffle=False):
    ...
    for i in range(0,len(name_im_gt_list)):
        ...
        dataloader=DataLoader(gos_dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers_)
...
def mask_preprocess(mask,size):
    if len(mask.shape) > 2:
        mask = mask[:, :, 0]

    mask_tensor = torch.unsqueeze(torch.tensor(mask, dtype=torch.uint8),0)

    if(len(size)<2):
        return mask_tensor.type(torch.uint8), mask.shape[0:2]
    else:
        mask_tensor = torch.unsqueeze(torch.tensor(mask_tensor, dtype=torch.float32),0)
        mask_tensor = F.upsample(mask_tensor, size, mode="bilinear")
...
class GOSRandomHFlip(object):
    ...
    def __call__(self,sample):
        imidx, image, label, mask, shape =  sample['imidx'], sample['image'], sample['label'], sample['mask'], sample['shape']
        ...
        return {'imidx':imidx,'image':image, 'label':label, 'mask':mask, 'shape':shape}
...
class GOSResize(object):
    ...
    def __call__(self,sample):
        ...
        mask = torch.squeeze(F.upsample(torch.unsqueeze(mask,0),self.size,mode='bilinear'),dim=0)
...
class GOSDatasetCache(Dataset):
    ...
    def __init__(self, name_im_gt_list, cache_size=[], cache_path='./cache', cache_file_name='dataset.json', cache_boost=False, transform=None):
        ...
        mask_path_list = [] # mask path
        im_ext_list = [] # im ext
        gt_ext_list = [] # gt ext
        mask_ext_list = [] # mask ext
        for i in range(0,len(name_im_gt_list)):
            ...
            mask_path_list.extend(name_im_gt_list[i]["mask_path"])
            ...
            mask_ext_list.extend([name_im_gt_list[i]["mask_ext"] for x in name_im_gt_list[i]["mask_path"]])
        ...
        self.dataset["mask_path"] = mask_path_list
        self.dataset["mask_im_path"] = deepcopy(mask_path_list)
        self.dataset["im_shp"] = []
        self.dataset["gt_shp"] = []
        self.dataset["mask_shp"] = []
        self.dataset["im_ext"] = im_ext_list
        self.dataset["gt_ext"] = gt_ext_list
        self.dataset["mask_ext"] = mask_ext_list


        self.dataset["ims_pt_dir"] = ""
        self.dataset["gts_pt_dir"] = ""
        self.dataset["masks_pt_dir"] = ""
    ...
    def cache(self,cache_folder):
        ...
        for i, im_path in tqdm(enumerate(self.dataset["im_path"]), total=len(self.dataset["im_path"])):
            ...
            if len(self.dataset["mask_path"])!=0:
                mask = im_reader(self.dataset["mask_path"][i])
            mask, mask_shp = mask_preprocess(mask,self.cache_size)
            mask_cache_file = os.path.join(cache_folder,self.dataset["data_name"][i]+"_"+im_id + "_mask.pt")
            torch.save(mask,mask_cache_file)
            if len(self.dataset["mask_path"])>0:
                cached_dataset["mask_path"][i] = mask_cache_file
            else:
                cached_dataset["mask_path"].append(mask_cache_file)
            if(self.cache_boost):
                masks_pt_list.append(torch.unsqueeze(mask,0))
            ...
            cached_dataset["mask_shp"].append(mask_shp)
        ...
        if(self.cache_boost):
            ...
            cached_dataset["masks_pt_dir"] = os.path.join(cache_folder, self.cache_boost_name+'_masks.pt')
            self.ims_pt = torch.cat(ims_pt_list,dim=0)
            self.gts_pt = torch.cat(gts_pt_list,dim=0)
            self.masks_pt = torch.cat(masks_pt_list,dim=0)
            ...
            torch.save(torch.cat(masks_pt_list,dim=0),cached_dataset["masks_pt_dir"])
    ...
    def load_cache(self, cache_folder):
        ...
        if(self.cache_boost):
            ...
            self.masks_pt = torch.load(dataset["masks_pt_dir"], map_location='cpu')
        return dataset
    ...
    def __getitem__(self, idx):
        ...
        if(self.cache_boost and self.ims_pt is not None):
            ...
            mask = self.masks_pt[idx]#.type(torch.float32)
            ...
            mask_pt_path = os.path.join(self.cache_path,os.sep.join(self.dataset["mask_path"][idx].split(os.sep)[-2:]))
            mask = torch.load(mask_pt_path)#(self.dataset["gt_path"][idx])
        ...
        im = torch.div(im,255.0)
        gt = torch.div(gt,255.0)
        mask = torch.div(mask,255.0)
